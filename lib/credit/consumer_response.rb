class Equifax::Credit::ConsumerResponse < ActiveRecord::Base
  ...
  def error
    ...
    if bca_error
      ...
      ("#{product_error["error_type"].humanize} error: #{product_error["input_container"]}, #{product_error["error_description"]}" rescue "There was an Equifax product error")
    else
      nil
    end
  end
  ...
  def number_of_commercial_defaults
    commercial_defaults.length
  rescue
    0
  end
