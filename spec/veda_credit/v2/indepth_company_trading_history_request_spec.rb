...
describe VedaCredit::V2::IndepthCompanyTradingHistoryRequest do
  ...
  describe 'with valid access, service and entity hash' do
    before(:all) do
      ...
      @request = VedaCredit::V2::IndepthCompanyTradingHistoryRequest.new(ref_id: 1, access: @access_hash, service: @service_hash, entity: @entity_hash, enquiry: @enquiry_hash)
      @request.save
    end
    ...
    describe '.service' do
      it 'returns product details hash used to build request' do
        expect(@request.service).to eq(service_code: 'XML2')
      end
    end
    ...
    describe '.xml' do
      it 'returns a soap request' do
        expect(@request.xml).to include('<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://vedaxml.com/vxml2/indepth-company-trading-history-v3-2.xsd" xmlns:wsa="http://www.w3.org/2005/08/addressing">')
      end
      ...
      it 'includes acn' do
        expect(@request.xml).to include('<com:australian-company-number>004139397</com:australian-company-number>')
      end
